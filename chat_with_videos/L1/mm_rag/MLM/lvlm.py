...
class LVLM(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if values["client"] is None:
            ...
            if url is None:
                hostname = get_from_dict_or_env(
                    values, "hostname", "VLM_HOSTNAME", None
                )
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and "prompt" in input.keys()
            and "image" in input.keys()
        ):
            return (
    ...
    def stream(
        ...
        **kwargs: Any,
        ...
        if type(self)._stream == BaseLLM._stream:
            ...
            for chunk in self._stream(prompt=prompt, image=image, **kwargs):
                yield chunk
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            async for chunk in self._astream(prompt=prompt, image=image, **kwargs):
                yield chunk
