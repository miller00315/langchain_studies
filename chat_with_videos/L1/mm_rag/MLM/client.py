...
class BaseClient(ABC):
    ...
    def generate_stream(self, prompt: str, image: str, **kwargs) -> Iterator[str]:
        """Send request to visual language model API
    ...
    def generate_batch(
        self, prompt: List[str], image: List[str], **kwargs
    ) -> List[str]:
...
class PredictionGuardClient(BaseClient):
    ...
    def generate(self, prompt: str, image: str, **kwargs) -> str:
        ...
        assert (
            image is not None and len(image) != ""
        ), "the input image cannot be None, it must be either base64-encoded image or path/URL to image"
        if isBase64(image):
            base64_image = image
